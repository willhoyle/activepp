...
Requestor::Requestor(const APISession& session, std::string& _dataFolder)
...
/*virtual*/ void Requestor::OnATTickHistoryDbResponse(uint64_t origRequest, ATTickHistoryResponseType responseType, LPATTICKHISTORY_RESPONSE pResponse)
{
	...
	std::cout << (dataFolder + "/" + date + "/" + symbol + ".csv").c_str() << std::endl;
	fileTrades.open((dataFolder + "/" + date + "/" + symbol + ".csv").c_str(), ios_base::app | ios_base::ate | ios_base::out);
	...
	std::cout << pResponse->nextOffset << std::endl;
